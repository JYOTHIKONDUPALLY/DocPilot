...
export const DriveService = {
 ...
 // List files in a specific folder
    listFilesInFolder : async (accessToken, folderId) => {
        try{
  ...
  // Get a specific file
  getFile: async (accessToken, fileId) => {
    try {
    ...
    getFileMetadata: async (accessToken, fileId) => {
        try {
          const response = await axios.get(
            `https://www.googleapis.com/drive/v3/files/${fileId}`, {
              params: {
  ...
  exportGoogleDoc: async (accessToken, fileId, mimeType = 'application/vnd.openxmlformats-officedocument.wordprocessingml.document') => {
    try {
      const response = await axios.get(
    ...
    // Create a new folder
    createFolder: async (accessToken, folderName, parentFolderId = null) => {
        try {
          const metadata = {
            name: folderName,
      ...
      createGoogleDoc: async (accessToken, title, content = '', parentFolderId = null) => {
        try {
